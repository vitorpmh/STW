...
annotated_data_face_only = pd.read_csv(
    os.path.join(main_path, "dissertation/dataset/anotations_cheeks_nose_only.csv")
)

# Read images and classes
images_path_cls = annotated_data_face_only[['paths_cheeks_nose', 'class']].values
...
for bins in [8, 16, 32, 64, 128]:
    ...
    with ProcessPoolExecutor(max_workers=10) as executor:
        results = list(tqdm(executor.map(process_image, images_path_cls), total=len(images_path_cls)))
    ...
    # Save CSV
    output_path = os.path.join(main_path, f"dissertation/CCVmodel/descriptors/descriptors_cheeks_nose_{bins}.csv")
